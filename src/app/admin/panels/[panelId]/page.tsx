...
import { getElectionRooms, deleteElectionRoom, getPanelById } from "@/lib/electionRoomService";
import type { ElectionRoom, ElectionPanel } from "@/lib/types";
...
import { PlusCircle, Settings, BarChart3, Users, CalendarDays, LockKeyhole, CheckCircle, Clock, XCircle, AlertTriangle, PenSquare, Vote, Star, Trash2, Loader2, Eye, EyeOff, ArrowLeft } from "lucide-react";
...
export default function PanelDetailPage() {
  ...
  const [panel, setPanel] = useState<ElectionPanel | null>(null);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        if (!panelId) {
            setError("Panel ID is missing.");
            setLoading(false);
            return;
        }
        try {
          const [panelData, roomsData] = await Promise.all([
              getPanelById(panelId),
              getElectionRooms(panelId)
          ]);
          ...
          if (err.code === 'permission-denied') {
            setError("You do not have permission to view this panel. Please contact support if you believe this is an error.");
          } else {
  ...
  return (
    ...
    <div className="space-y-8">
      ...
      <div className="text-center">
          <h1 className="text-3xl font-bold font-headline">{panel.title}</h1>
          <p className="text-muted-foreground mt-2 max-w-2xl mx-auto">{panel.description}</p>
      </div>

      <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
        <Button asChild variant="secondary">
          <Link href={`/admin/rooms/create-review?panelId=${panel.id}`}>
            <PenSquare className="mr-2 h-5 w-5" /> Create New Review Room
          </Link>
        </Button>
        <Button asChild>
          <Link href={`/admin/rooms/create?panelId=${panel.id}`}>
      ...
      {electionRooms.length > 0 ? (
        ...
        <Card className="text-center py-16">
          <CardHeader>
            ...
            <CardDescription>Get started by creating a new voting or review room within this panel.</CardDescription>
          </CardHeader>
        </Card>
      )}
